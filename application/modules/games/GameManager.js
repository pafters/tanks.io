...
class GameManager extends BaseModule {
    ...
    _createRoom = (socket) => { //метод создания комнаты
        ...
        while (flag) { //он будет срабатывать до тех пор, пока не подберется свободный id
            ...
            if (index == - 1) { //если не находим
                ...
                this._sendRoomInfo(room, socket); //отправляем на клиенты список актуальных комнат
    ...
    */
    ...
    _joinRoom = (data, socket) => { //метод присоединения к комнате
        ...
        this.io.to(joinTag).emit(this.SOCKET_MESSAGES.SHOW_ROOM, data)  //Отправляем на клиенты пользаков, которые находятся в этой комнате, обновленный список пользаков, находящихся в ней
    }
